"""
gnn_models_v2.py — SupraGraphNet para predicción de supervivencia (HNSC)  multiclase
...
class SupraGraphNetConfig:
    ...
    use_hpv:        bool  = True
    hpv_embed_dim:  int   = 8
    num_classes:    int   = 2   # 2=binario, 3=3 clases literatura, 4=4 clases literatura
...
class SupraGraphNet(nn.Module):
    """
    SupraGraphNet v8 — clasificación con Focal Loss + Label Smoothing.
    Arquitectura idéntica a v7. Solo cambia la función de loss y la cabeza de salida.
    - num_classes=2 (binario): forward() devuelve logits [n_patients].
      sigmoid(logits) = P(supervivencia larga).
    - num_classes>2 (multiclase): forward() devuelve logits [n_patients, num_classes].
      softmax(logits) = distribución de probabilidad sobre clases.
    """
    ...
    def __init__(self, config: SupraGraphNetConfig):
        ...
        self.os_head = nn.Sequential(
            ...
            nn.Linear(config.os_head_dim, config.num_classes if config.num_classes > 2 else 1),
        )
    ...
    def forward(self, data: HeteroData, hpv: Optional[torch.Tensor] = None,
        ...
        raw = self.os_head(g_patient)
        # binario: [N,1] → squeeze a [N]; multiclase: [N,C] queda como [N,C]
        logits = raw.squeeze(-1) if self.config.num_classes <= 2 else raw
...
def focal_loss_with_smoothing(
    ...
    """
    Focal Loss + Label Smoothing unificada para binario y multiclase.

    - num_classes=2 → binario: BCE focal con label smoothing (idéntico a v1)
    - num_classes>2 → multiclase: CrossEntropy focal con label smoothing uniforme

    Parameters
    ----------
    logits : Tensor [n] (binario) o [n, C] (multiclase)
    labels : Tensor [n] — enteros 0..C-1
    gamma  : factor de focusing (0=CE pura)
    smoothing : label smoothing (0=sin suavizado)
    num_classes : nº de clases
    """
    if num_classes <= 2:
        # ── Binario (comportamiento original) ─────────────────────────
        labels_f = labels.float()
        labels_smooth = labels_f * (1.0 - smoothing) + smoothing * 0.5
        probs   = torch.sigmoid(logits)
        probs_t = torch.where(labels_f >= 0.5, probs, 1.0 - probs)
        bce     = F.binary_cross_entropy_with_logits(logits, labels_smooth, reduction='none')
        loss    = ((1.0 - probs_t).pow(gamma) * bce).mean()
        return loss
    else:
        # ── Multiclase: focal CE con label smoothing ───────────────────
        labels_long = labels.long()
        # Label smoothing: distribuye smoothing/C a todas las clases
        C = num_classes
        with torch.no_grad():
            smooth_labels = torch.full(
                (labels_long.size(0), C), smoothing / C, device=logits.device
            )
            smooth_labels.scatter_(1, labels_long.unsqueeze(1), 1.0 - smoothing + smoothing / C)
        log_probs = F.log_softmax(logits, dim=-1)
        probs     = log_probs.exp()
        # p_t = probabilidad de la clase correcta
        p_t      = probs.gather(1, labels_long.unsqueeze(1)).squeeze(1)
...
def compute_classification_metrics(
    ...
    - num_classes=2: binario, logits_np shape [N], AUC estándar
    - num_classes>2: multiclase, logits_np shape [N, C], AUC OvR macro, F1 macro
    """
    ...
    if num_classes <= 2:
        ...
        # Multiclase
        probs = sp_softmax(logits_np, axis=-1)   # [N, C]
...
def train_supra_graph_classifier(
    ...
    """
    Entrena SupraGraphNet v8 con Focal Loss + Label Smoothing.
    ...
    data   = data.to(device)
    # Para multiclase la loss necesita long; para binario float
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        # Focal Loss + Label Smoothing (binario o multiclase según config)
...
def make_optuna_objective_classifier(
    ...
    - num_classes=2: binario (original), minimiza 1-AUC binario
    - num_classes>2: multiclase, minimiza 1-AUC OvR macro
    """
...
def run_gnnexplainer(
    ...
    for pat_idx in patient_indices:
        try:
            ...
            # Para multiclase logits[pat_idx] es [C], no escalar → usar max (logit clase predicha)
            scalar = logits[pat_idx] if logits.dim() == 1 else logits[pat_idx].max()
            scalar.backward()
